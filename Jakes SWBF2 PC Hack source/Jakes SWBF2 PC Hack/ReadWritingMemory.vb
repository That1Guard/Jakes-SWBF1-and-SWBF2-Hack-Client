Module ReadWritingMemory
    ...
    Public Sub WriteInteger(ByVal ProcessName As String, ByVal Address As Integer, ByVal Value As Integer, Optional ByVal nsize As Integer = 4)
        ...
        Dim hProcess As IntPtr = OpenProcess(PROCESS_ALL_ACCESS, 0, MyP(0).Id)
        If hProcess = IntPtr.Zero Then
            MessageBox.Show("Failed to toggle hack state! Check to make sure there are no other hacks hooked onto the process or check to make sure an anti-virus program is not blocking the hack. If the issue persists, try launching star wars with the launch game button on the hack.")
            Exit Sub
        End If
